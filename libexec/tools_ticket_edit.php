	...
	******************************************************************/
	...
	if(isset($_POST['ajax']) && isset($_POST['id'])) {
		...
		if(isset($_POST['message'])) {
			...
			if($percent > 30) $message = ucfirst(mb_strtolower($message, 'UTF-8'));
			// --- Prepare query for update
			$query = 'UPDATE at_ticket_messages SET message = $1 WHERE id = $2';
			$params = array(trim($message), intval($_POST['id']));
			if($_pgobj->query_params($query, $params)) echo trim($message);
			else echo $_msg->wrn("Couldn't update message!");
		}
		// --- Case Edit is Delete
