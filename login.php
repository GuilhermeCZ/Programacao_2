...
// ----- Setting Session Variables ----- //
	class session {
		function __construct() {
			...
			$this->error = FALSE;
			$this->timeout = intval($_settings->config['Session Timeout']);
		}
	} $_session = new session();
	$posted_form = (isset($_POST['nam']) && isset($_POST['pas'])) ? (TRUE) : (FALSE);
...
// ----- Going ahead if not signed in yet ----- //
	if(!isset($_SESSION['php_session_id'])) {
		...
		if($_pgobj->rows == 0) {
			...
			$_pgobj->query_params($query, array(($posted_form) ? ($_POST['nam']) : (NULL), $_session->php_session_id));
		}
// ----- Trying to sign in if is there a form submited ----- //
		if($posted_form) {
			...
			$_pgobj->query_params($query, array($_POST['nam'], $_POST['pas']));
			if($_pgobj->rows == 1) {
