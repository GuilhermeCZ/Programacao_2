	...
	********************************************************/
	...
	// Initialize log file //
		function __construct() {
			...
			if(!isset($_path->conf)) { // dependency not met
				...
				else $_msg->wrn("Can't write to log file!");
				return FALSE;
			}
	...
	// Write to Log Function //
		function write($type, $action, $info) {
			...
			if(!isset($this->handle)) return $_msg->wrn("Can't write to log file!");
			if(!is_resource($this->handle)) return $_msg->wrn("Can't write to log file!");
			$date_now = date("Ymd His");
			$username = (isset($_sessiob->username)) ? ($_session->username) : ($_msg->lang('unknown'));
