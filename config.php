...
// ----- Guessing User Language ----- //
	...
	if(!file_exists("$_path->lang/$browser_main_language.lng")) {
		...
		if(count($temp)>0) $browser_main_language = substr($temp[0], (strrpos($temp[0], '/') + 1), -4);
	} unset($temp);
// ----- Defining Messages Class ----- //
	require("$_path->php/msgs.php");
	$_msg = new msgs($browser_main_language);
// ----- Verifying DataBase Config File ----- //
	if(!file_exists("$_path->conf/database.pgpass")) $_msg->err("Couldn't find DataBase!");
// ----- Connecting Database ----- //
	$db_dsn_array = explode(':', file_get_contents("$_path->conf/database.pgpass"));
	$db_dsn_string = 'host='.$db_dsn_array[0].' port='.$db_dsn_array[1].' dbname='.$db_dsn_array[2];
	$db_dsn_string.= ' user='.$db_dsn_array[3].' password='.$db_dsn_array[4];
	require("$_path->php/pgsql.php");
	$_pgobj = new pgsql($db_dsn_string);
	if($_pgobj->error == "FATAL") $_msg->err("Couldn't connect to the DataBase!");
