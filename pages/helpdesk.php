	...
	*******************************************************************/
	...
	$success = FALSE;
	$date_format = (isset($_settings->config["Date Format"])) ? ($_settings->config["Date Format"]) : ("m/d/Y");
	// ----- Formating Date ----- //
	...
	if(isset($_POST['deadline'])) {
		...
		if($deadline > (strtotime("+2 weeks") + $final_time_diff)) $_msg->err("Invalid Deadline!");
		elseif($deadline < strtotime("-1 month")) $_msg->err("Invalid Deadline!");  // Make it configurable... later//
...
<?php
	...
	if(isset($_POST['customer_id'])) {
		...
		else $_msg->err("Could not open ticket!");
	...
	if(isset($_POST['ticket_id'])) {
		...
		if($_pgobj->rows == 0) $_msg->err("Could not add message to the ticket!");
		...
		else $_msg->err("Could not update ticket!");
	}
...
<?php	}
		...
		if(isset($_GET['tid'])) {
			...
			if($_pgobj->rows == 0) $_msg->err("Could not open ticket!");
...
<?php		if($_session->groupname != 'tech') { ?>
											<div class="btn-group">
												<button id="details_disable" class="btn btn-default"><?= $_msg->lang("Disable"); ?></button>
